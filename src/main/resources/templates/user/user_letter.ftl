    ...
    <div class="row" >
        <div class="col-lg-9 col-md-12 col-sm-12" >
            <ul class="nav nav-tabs">
                ...
                <li role="presentation"><a href="/user/follows?userId=${toUser.id?c}">关注的人</a></li>
                <li role="presentation"><a href="/user/fans?userId=${toUser.id?c}">粉丝</a></li>
                <#if !user.id??>
                <#elseif toUser.id?c = user.id?c>
                    <li role="presentation" class="active"><a href="/user/letter">私信</a></li>
                </#if>
            </ul>
            <hr>
            <#list listMessage as message>
                <div class="media">
                    <div class="media-left">
                        <img class="media-object img-rounded" src="${message.headerUrl}" alt="#" style="height: 40px">
                    </div>
                    <div class="media-body">
                        <h4 class="media-heading"><a href="/user/chat?userId=${message.otherUserId}">${message.username}</a></h4>
                        <span style="float: right">${message.createTime?string("yyyy-MM-dd HH:mm:ss")}</span>
